...
func needsRefresh(existingCert *x509.Certificate, now time.Time, refresh time.Duration, desiredCert *x509.Certificate, issuerPublicKey *rsa.PublicKey) string {
...
func extractExistingSecret(
	...
	refreshReason = needsRefresh(cert, now, refresh, desiredCert, desiredIssuerKey)
	if len(refreshReason) != 0 {
		return nil, nil, nil, refreshReason
	}
...
func makeCertificate(name string, certCreator ocrypto.CertCreator, signer ocrypto.Signer, validity, refresh time.Duration, controller metav1.Object, controllerGVK schema.GroupVersionKind, existingSecret *corev1.Secret) (*TLSSecret, error) {
	...
	if len(refreshReason) != 0 {
		klog.V(2).InfoS("Creating certificate", "Object", naming.ObjRef(tlsSecret.GetSecret()), "Reason", refreshReason)
	...
	for annotationKey, annotationFunc := range CertKeyProjectedAnnotations {
		tlsSecret.GetSecret().Annotations[annotationKey], err = annotationFunc(certs, &key.PublicKey)
		if err != nil {
...
func MakeCA(name string, certCreator ocrypto.CertCreator, signer ocrypto.Signer, validity, refresh time.Duration, issuerSecret *corev1.Secret, controller metav1.Object, controllerGVK schema.GroupVersionKind, existingSecret *corev1.Secret) (*SigningTLSSecret, error) {
	if issuerSecret == nil {
		return nil, fmt.Errorf("missing issuer CA secret")
	}
	...
	return NewSigningTLSSecret(tlsSecret, signer.Now), nil
}

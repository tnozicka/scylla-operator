...
var (
	...
	CertKeyProjectedAnnotations = helpers.MergeMaps(
		...
		map[string]func([]*x509.Certificate, *rsa.PublicKey) (string, error){},
	)
)
...
func wrapCertProjectionsForCertKey(
	...
	res := make(map[string]func([]*x509.Certificate, *rsa.PublicKey) (string, error), len(certProjections))

	for k, fi := range CertProjectedAnnotations {
		f := fi
		res[k] = func(certificates []*x509.Certificate, key *rsa.PublicKey) (string, error) {
...
func GetCertsKeyDataFromSecret(secret *corev1.Secret) ([]byte, []byte, error) {
	certBytes, err := GetCertDataFromSecret(secret)
	if err != nil {
		return nil, nil, fmt.Errorf("can't get certificate bytes from secret %q: %w", naming.ObjRef(secret), err)
	}

	keyBytes := secret.Data[corev1.TLSPrivateKeyKey]
	if len(keyBytes) == 0 {
		return nil, nil, fmt.Errorf("secret %q is missing certificate key", naming.ObjRef(secret))
	}
...
func NewTLSSecretWithCache(secret *corev1.Secret, certs []*x509.Certificate, key *rsa.PrivateKey) *TLSSecret {
	return &TLSSecret{
		secret: secret,
		certs:  certs,
		key:    key,

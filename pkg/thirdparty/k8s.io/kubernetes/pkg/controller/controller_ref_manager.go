...
// Three methods are defined on this object 1: Classify 2: AdoptStatefulSet and
// 3: ReleaseStatefulSet which are used to classify the ReplicaSets into appropriate
...
// ClaimStatefulSets tries to take ownership of a list of ReplicaSets.
//
// It will reconcile the following:
//   * Adopt orphans if the selector matches.
//   * Release owned objects if the selector no longer matches.
...
// returned.
func (m *ReplicaSetControllerRefManager) ClaimStatefulSets(sets []*apps.ReplicaSet) ([]*apps.ReplicaSet, error) {
	...
	adopt := func(obj metav1.Object) error {
		return m.AdoptStatefulSet(obj.(*apps.ReplicaSet))
	}
	release := func(obj metav1.Object) error {
		return m.ReleaseStatefulSet(obj.(*apps.ReplicaSet))
	}
...
// AdoptStatefulSet sends a patch to take control of the ReplicaSet. It returns
// the error if the patching fails.
func (m *ReplicaSetControllerRefManager) AdoptStatefulSet(rs *apps.ReplicaSet) error {
	if err := m.CanAdopt(); err != nil {
...
// ReleaseStatefulSet sends a patch to free the ReplicaSet from the control of the Deployment controller.
// It returns the error if the patching fails. 404 and 422 errors are ignored.
func (m *ReplicaSetControllerRefManager) ReleaseStatefulSet(replicaSet *apps.ReplicaSet) error {

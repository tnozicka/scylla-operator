...
func (scc *Controller) syncCerts(
	...
	for _, svc := range serviceMap {
		...
		if len(hostID) == 0 {
			...
			// TODO: this needs to be in an outer loop or it complains twice.
	...
	if len(progressingMessages) != 0 {
		apimeta.SetStatusCondition(&status.Conditions, metav1.Condition{
			Type:               scyllav1.CertControllerProgressingCondition,
			Status:             metav1.ConditionTrue,
			Reason:             scyllav1.ProgressingReason,
	...
	return status, apierrors.NewAggregate(errors)
}

...
func (a *RackScaleDown) Execute(ctx context.Context, s *State) error {
	...
	if memberService.Labels[naming.DecommissionedLabel] == naming.LabelValueTrue {
		return errors.WithStack(a.completeScaleDown(ctx, s))
	}

	if _, ok := memberService.Labels[naming.DecommissionedLabel]; r.Members < c.Status.Racks[r.Name].Members && !ok {
...
func (a *RackScaleDown) beginScaleDown(ctx context.Context, s *State, memberService *corev1.Service) error {
	...
	memberService.Labels[naming.DecommissionedLabel] = naming.LabelValueFalse

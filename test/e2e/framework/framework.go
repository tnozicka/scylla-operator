...
func NewFramework(namePrefix string) *Framework {
	...
	f.FullClient.AdminClient.Config = f.GetDefaultCluster().AdminClientConfig()
...
func (f *Framework) Namespace() string {
	return f.GetDefaultCluster().defaultNamespace.Name
}

func (f *Framework) DefaultNamespaceIfAny() (*corev1.Namespace, Client, bool) {
	return f.GetDefaultCluster().DefaultNamespaceIfAny()
}
...
func (f *Framework) AddCleaners(cleaners ...CleanupInterface) {
	f.GetDefaultCluster().AddCleaners(cleaners...)
}

func (f *Framework) CreateUserNamespace(ctx context.Context) (*corev1.Namespace, Client) {
	return f.GetDefaultCluster().CreateUserNamespace(ctx)
}
...
func (f *Framework) GetDefaultCluster() *Cluster {
	o.Expect(f.clusters).NotTo(o.BeEmpty())
	return f.clusters[0]
}

func (f *Framework) beforeEach(ctx context.Context) {
	ns, nsClient := f.GetDefaultCluster().CreateUserNamespace(ctx)
	f.GetDefaultCluster().defaultNamespace = ns
	f.GetDefaultCluster().defaultClient = nsClient
	f.FullClient.Client = nsClient
}

func (f *Framework) justAfterEach(ctx context.Context) {
	ginkgoNamespace := f.GetDefaultCluster().defaultNamespace.Name
...
func (f *Framework) afterEach(ctx context.Context) {
	...
	f.GetDefaultCluster().defaultNamespace = nil
	f.GetDefaultCluster().defaultClient = nilClient

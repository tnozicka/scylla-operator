...
var _ = g.Describe("ScyllaCluster", func() {
	...
	g.It("should setup and maintain up to date TLS certificates", func() {
		...
		framework.By("Filling the cluster with data")
	...
	g.It("should rotate TLS certificates before they expire", func() {
		...
		for _, item := range items {
			func() {
				...
				framework.By("Replacing secret %q cert to be past its latest refresh time (80%%)", item.secretName)
				...
				validity := 80 * time.Hour
				cert.NotAfter = now.Add(validity / 5)
				cert.NotBefore = now.Add(-validity * 4 / 5)
				...
				waitCtxL1, waitCtxL1Cancel := context.WithTimeout(ctx, 30*time.Second)
				...
				if len(item.cmName) != 0 {
					...
					waitCtxL2, waitCtxL2Cancel := context.WithTimeout(ctx, 30*time.Second)
